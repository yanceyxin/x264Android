...
static void mb_analyse_intra( x264_t *h, x264_mb_analysis_t *a, int i_satd_inter )
{
    ...
    /*creat by yancey on 2020.5.20*/
    const int t0 = 1699;/*just for crf = 21~24*/
    const int t1 = 4599;
    const int t2 = 7599;
    const int t3 = 9999;
    int _mean = 0;
    ...
    if(_16mode){
        if(!h->param.i_avcintra_class){
            ...
            static const uint8_t i16x16_thresh_lut[11] = {2,3,4,4,5,5,5,5,6,6,6};
...
static void mb_analyse_intra_p84( x264_t *h, x264_mb_analysis_t *a, int i_satd_inter)
{
    const unsigned int flags = h->sh.i_type == SLICE_TYPE_I ? h->param.analyse.intra : h->param.analyse.inter;
    pixel *p_src = h->mb.pic.p_fenc[0];
    pixel *p_dst = h->mb.pic.p_fdec[0];
    ...
    int lambda = a->i_lambda;
    /*only for i8x8/i4x4 */
    int t1 = 75*(h->mb.i_qp-10);
    int t2 = 25*h->mb.i_qp+1400;
    int t3 = 188*h->mb.i_qp+1500;
    int _8mode = 0;
    int _4mode = 0;
    if(a->i_satd_i16x16 < t1){
        _8mode = 0;
        _4mode = 0;
    }else{
        if(a->i_satd_i16x16 < t2){
            _8mode = 1;
            _4mode = 0;
        }
        else{
            if(a->i_satd_i16x16 < t3){
                _8mode = 1;
                _4mode = 1;
            }
            else{
...
skip_analysis:
        ...
        else
        {
            ...
            /*created by yancey on 2020.5.26*/
            mb_analyse_intra_p16(h, &analysis, i_cost);
            int _thresh = 4*pow(2, 5 + h->mb.i_qp/6);
            int _auxMode = 1;
            int _left = 0;
            int _top = 0;
            int _topleft = 0;
            int _topright = 0;
            if(h->mb.i_mb_type_left[0] == I_16x16 || h->mb.i_mb_type_left[0] == I_8x8 || h->mb.i_mb_type_left[0] == I_4x4){
                _left = 1;
            }
            if(h->mb.i_mb_type_top == I_16x16 || h->mb.i_mb_type_top == I_8x8 ||h->mb.i_mb_type_top == I_4x4){
                _top = 1;
            }
            if(h->mb.i_mb_type_topleft == I_16x16 || h->mb.i_mb_type_topleft == I_8x8 ||h->mb.i_mb_type_topleft == I_4x4){
                _topleft = 1;
            }
            if(h->mb.i_mb_type_topright == I_16x16 || h->mb.i_mb_type_topright == I_8x8 ||h->mb.i_mb_type_topright == I_4x4){
                _topright = 1;
            }
            ...
            if( analysis.i_mbrd || !h->mb.i_subpel_refine ){}
                ...
                i_cost = analysis.l0.me8x8[3].cost;
            }
            if(_auxMode){
                 mb_analyse_intra_p84( h, &analysis, i_cost );
            }
